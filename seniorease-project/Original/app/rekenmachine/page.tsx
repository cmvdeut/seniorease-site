...
export default function RekenmachinePage() {
  ...
  const Button = ({ onClick, children, className = '' }: { onClick: () => void; children: React.ReactNode; className?: string }) => (
    <button
      onClick={onClick}
      className={`${className} bg-white border-4 border-primary rounded-xl text-senior-2xl font-bold
                 hover:bg-primary/10 active:scale-95 transition-all shadow-lg
                 min-h-[80px] min-w-[80px] flex items-center justify-center`}
    >
      {children}
    </button>
  );
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl border-4 border-primary p-6 md:p-8">
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <Button onClick={() => performOperation('÷')} className="bg-accent text-white border-accent hover:bg-accent/80">
                ÷
              </Button>
              <Button onClick={() => performOperation('×')} className="bg-accent text-white border-accent hover:bg-accent/80">
                ×
